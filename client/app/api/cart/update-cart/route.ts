...
async function updateCart(cookieHeader: string, cartData: any) {
  try {
    const response = await axios.post(`${serverUrl}/cart/update-cart`, cartData, {
      headers: {
        Cookie: cookieHeader,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await updateCart(cookieHeader, cartData);
    ...
    return NextResponse.json(
      { 
        success: false, 
        message: getErrorMsg(error, null, "updating cart")
      },
