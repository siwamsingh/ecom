...
async function changeDefaultAddress(cookieHeader: string, addressId: any) {
  try {
    const response = await axios.post(`${serverUrl}/address/change-default-address`, {address_id: addressId}, {
      headers: {
        Cookie: cookieHeader,
...
export async function POST(request: NextRequest) {
  ...
  try {
    // Get request body
     addressId = await request.json();
     
    // Send the address data to the backend with cookies
    const result = await changeDefaultAddress(cookieHeader, addressId);
    ...
    return NextResponse.json(
      { 
        success: false, 
        message: getErrorMsg(error, null, "changing default address")
      },
