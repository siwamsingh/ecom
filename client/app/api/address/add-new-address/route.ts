...
async function addAddressToServer(cookieHeader: string, addressData: any) {
  try {
    const response = await axios.post(`${serverUrl}/address/add-new-address`, addressData, {
      headers: {
        Cookie: cookieHeader,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await addAddressToServer(cookieHeader, addressData);
    ...
    return NextResponse.json(
      { 
        success: false, 
        message: getErrorMsg(error, null, "adding new address")
      },
