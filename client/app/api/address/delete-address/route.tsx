...
async function deleteAddress(cookieHeader: string, addressId: any) {
  try {
    const response = await axios.post(`${serverUrl}/address/delete-address`, {address_id: addressId}, {
      headers: {
        Cookie: cookieHeader,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await deleteAddress(cookieHeader, addressId);
    ...
    return NextResponse.json(
      { 
        success: false, 
        message: getErrorMsg(error, null, "deleting address address")
      },
